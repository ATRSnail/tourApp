...
//        view.setHasFixedSize(true);
    ...
    public static void initRecyclerViewG(Context context, RecyclerView view, boolean isDivided,
                                         RecyclerView.Adapter adapter, int column, int spacingRec, boolean hasHeader) {
        GridLayoutManager layoutManager = new GridLayoutManager(context, column, LinearLayoutManager.VERTICAL, false);
        view.setLayoutManager(layoutManager);
        view.setItemAnimator(new DefaultItemAnimator());
        if (isDivided) {
            view.addItemDecoration(new MarginDecoration(context, column, true, spacingRec, hasHeader ? 1 : 0));
        }
        view.setAdapter(adapter);
    }

    public static void initRecyclerViewG(Context context, RecyclerView view, boolean isDivided,
                                         RecyclerView.Adapter adapter, int column, int spacingRec) {
        initRecyclerViewG(context, view, isDivided, adapter, column, spacingRec, false);
    }
